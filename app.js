...
function confidenceBadge(bot, rank) { const confidence = botConfidence(bot); return `<div class="confidence-badge score-${Math.floor(confidence.score / 20)}" title="Evidence-weighted score; not a forecast or profit guarantee"><b>#${rank}</b><span>${confidence.score}/100</span><small>${confidence.label}</small></div>`; }
...
async function showBotDetails(botId) {
  ...
  const confidence = botConfidence(bot), confidencePanel = `<h3 class="detail-title">Bot Confidence Score</h3><div class="confidence-panel"><div class="confidence-total"><strong>${confidence.score}<small>/100</small></strong><span>${confidence.label}</span><p>Evidence-weighted ranking, not a profit forecast.</p></div><div class="confidence-breakdown">${Object.entries(confidence.breakdown).map(([label,value])=>`<div><span>${escapeHtml(label)}</span><div><i style="width:${Math.min(100,Math.abs(Number(value))/25*100)}%" class="${Number(value)<0?"penalty":""}"></i></div><strong>${Number(value)>=0?"+":""}${Number(value).toFixed(1)}</strong></div>`).join("")}</div><div class="confidence-evidence"><span>${confidence.coverageDays.toFixed(0)} test days</span><span>${confidence.observations} ${bot.asset_class==="option"?"signals":"trades"}</span><span>${confidence.fillCount} paper fills</span><span>${confidence.validationAverage==null?"No unseen result":`${pct(confidence.validationAverage)} unseen average`}</span><span>${confidence.siblingCount} family candidates tested</span></div></div>`;
  ...
  $("#modal-content").innerHTML = `<div class="modal-head"><div><h3>${escapeHtml(bot.name)}</h3><p>${escapeHtml(bot.symbol)} · ${escapeHtml(bot.bot_type.replaceAll("_", " "))}</p></div><button type="button" class="icon-button" data-close-modal>×</button></div><div class="modal-body">${generated}${confidencePanel}<div class="detail-grid"><div><span>Maximum allocation</span><strong>${money(bot.max_allocation)}</strong></div><div><span>Take profit</span><strong>${bot.take_profit_pct ? pct(bot.take_profit_pct) : "Spread rule"}</strong></div><div><span>Stop loss</span><strong>${bot.stop_loss_pct ? pct(bot.stop_loss_pct) : "Defined by spread"}</strong></div><div><span>Session</span><strong>${escapeHtml(bot.session_policy)}</strong></div></div>${riskMetrics}${tickerInfo}${tickerNews}<h3 class="detail-title">Start rules (${escapeHtml(rule.operator || "AND")})</h3><div class="rule-list">${conditions.map((condition) => `<div><strong>${escapeHtml(conditionDefinition(condition.type)[1])}</strong><span>${escapeHtml(condition.timeframe || "")} · ${escapeHtml(Object.entries(condition.parameters || {}).map(([key, value]) => `${key}: ${value}`).join(", "))}</span></div>`).join("") || "No structured rules saved."}</div>${generatedValues}<div class="section-head"><h3>${bot.asset_class === "option" ? "Signal-test history" : "Backtest history"}</h3><span class="subtle">${formatDuration(backtestSummary.get(bot.id)?.seconds || 0)} total coverage</span></div>${history}</div><div class="modal-foot"><button class="secondary" data-close-modal>Close</button><button class="primary" data-backtest="${bot.id}">${bot.asset_class === "option" ? "Test signals" : "Backtest"}</button></div>`;
  modal.showModal();
}
