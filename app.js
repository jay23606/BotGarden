...
async function loadDashboard() {
  ...
  content.innerHTML = `
    ...
    <div class="runner-note"><strong>Paper execution worker active.</strong> ON stock bots are evaluated every five minutes and may submit Alpaca paper orders when every start condition matches. Option strategies remain signal-only and fail closed until multi-leg execution is enabled.</div>
...
function switchView(view) {
  ...
  content.innerHTML = `<div class="section-head"><h3>Broker connections</h3></div><div class="card connection-card"><p>Connect an Alpaca paper account using credentials created in your Alpaca dashboard.</p><button class="primary" data-connect>Connect Alpaca</button></div><div class="section-head"><h3>Paper execution capacity</h3></div><div class="settings-grid"><div class="card"><span class="eyebrow">ACTIVE SCHEDULE</span><h3>Every five minutes</h3><p>The shared worker evaluates ON stock bots during Alpaca market hours. OFF bots cannot create new orders.</p></div><div class="card"><span class="eyebrow">ORDER SAFETY</span><h3>Idempotent paper orders</h3><p>Deterministic client order IDs prevent duplicate submissions within each bot cooldown window.</p></div><div class="card"><span class="eyebrow">DATABASE POLICY</span><h3>Meaningful events only</h3><p>Signals that become orders, broker responses, and errors are retained; ordinary failed condition checks are not written.</p></div><div class="card"><span class="eyebrow">CURRENT SCOPE</span><h3>Stocks execute; options fail closed</h3><p>Option bots continue signal testing but cannot submit orders until contract selection and multi-leg exits are implemented.</p></div></div><div class="section-head"><h3>Backtest data notes</h3></div><div class="card note-card"><h3>Why option bots show “Signal-only”</h3><p>Stock bots can be simulated directly from historical bars. Option strategies require historical contract chains and Greeks to reproduce selection accurately, so BotGarden records underlying triggers without inventing option P&amp;L.</p></div>`;
}
