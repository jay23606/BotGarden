...
function renderCrypto() {
  ...
  const rows = cryptoBots.map((bot) => {
    ...
    return `<div class="bot-row"><div><div class="bot-name">${escapeHtml(bot.name)}</div><div class="subtle">${escapeHtml(bot.symbol)} · ${escapeHtml(label)} · 24/7</div></div><button class="bot-toggle ${isOn ? "on" : ""}" data-toggle-bot="${bot.id}" role="switch" aria-checked="${isOn}"><span></span>${isOn ? "ON" : "OFF"}</button><div><div class="subtle">HISTORICAL REPLAY</div><strong class="${summary?.profitPct > 0 ? "profit" : summary?.profitPct < 0 ? "loss" : ""}">${result}</strong><div class="subtle">${formatDuration(summary?.seconds || 0)}</div></div>${stockSparkline(bot)}<div><div class="subtle">MAX ALLOCATION</div>${money(bot.max_allocation)}</div><div class="row-actions"><button class="icon-action" data-bot-details="${bot.id}" title="View strategy details">${actionIcon("details")}</button><button class="icon-action test" data-backtest="${bot.id}" title="Replay this strategy">${actionIcon("test")}</button><button class="delete-button" data-delete-bot="${bot.id}" title="Remove bot">×</button></div></div>`;
  }).join("");
...
async function createCryptoStrategy(strategy,risk,days,asset){
  ...
  if(strategy==="crypto_dca"){const safetyCount=pick([3,4,5]),initial=Math.max(10,Math.floor(risk*pick([.18,.2,.22,.25]))),remaining=risk-initial,scale=pick([1.15,1.25,1.35]),weights=Array.from({length:safetyCount},(_,i)=>Math.pow(scale,i)),weightTotal=weights.reduce((a,b)=>a+b,0),deviationStep=Math.max(1.2,Math.min(5,atr*pick([.65,.8,1]))),steps=[{step:0,deviation:0,amount:initial},...weights.map((weight,i)=>({step:i+1,deviation:Number((deviationStep*Math.pow(i+1,1.18)).toFixed(2)),amount:Number((remaining*weight/weightTotal).toFixed(2))}))];config={safety_orders:safetyCount,deviation_step_pct:Number(deviationStep.toFixed(2)),volume_scale:scale,steps};conditions=[{type:"rsi",timeframe:"5Min",parameters:{period:14,operator:"below",value:pick([38,40,42,45])}}];botType="dca";name=`${base} Smart DCA`;takeProfit=pick([2,2.5,3,4,5]);stopLoss=pick([10,12,15]);cooldown=1800;}
  ...
  else if(strategy==="crypto_regime_allocator"){config={target_vol_pct:pick([2.5,3,3.5,4]),risk_off_atr_pct:Math.max(4,Math.min(10,atr*1.4)),min_edge_bps:pick([50,65,80])};conditions=[{type:"moving_average",timeframe:"1Hour",parameters:{average:"ema",fast:20,slow:50,operator:"above"}},{type:"atr",timeframe:"1Hour",parameters:{period:14,operator:"below",value:config.risk_off_atr_pct}}];name=`${base} Regime Allocator`;takeProfit=pick([6,8,10,12]);stopLoss=pick([4,5,6]);}
  else if(strategy==="crypto_vol_momentum"){config={target_vol_pct:pick([2,2.5,3]),min_edge_bps:pick([60,75,90])};conditions=[{type:"moving_average",timeframe:"1Hour",parameters:{average:"ema",fast:12,slow:36,operator:"above"}},{type:"rsi",timeframe:"1Hour",parameters:{period:14,operator:"above",value:pick([52,55,58])}},{type:"atr",timeframe:"1Hour",parameters:{period:14,operator:"below",value:Math.max(4,Math.min(10,atr*1.5))}}];name=`${base} Volatility Momentum`;takeProfit=pick([6,8,10]);stopLoss=pick([4,5,6]);}
  ...
  else{config={interval_hours:pick([6,8,12,24,48]),installments:pick([5,6,8,10,12]),volatility_scale:atr>6?.6:atr>3?.8:1};conditions=[{type:"immediate",timeframe:"1Hour",parameters:{}}];name=`${base} Volatility Accumulator`;takeProfit=pick([12,15,18,20]);cooldown=config.interval_hours*3600;}
...
async function showCryptoBatchFormV3(){
  const families=[{id:"crypto_dca",label:"DCA + safety orders",weight:32},{id:"crypto_regime_allocator",label:"Regime-gated allocator",weight:24},{id:"crypto_vol_momentum",label:"Volatility momentum",weight:16},{id:"crypto_shock_recovery",label:"Shock recovery",weight:12},{id:"atr_adaptive_grid",label:"Adaptive grid",weight:8},{id:"crypto_smart_trailing",label:"Smart trailing",weight:4},{id:"crypto_scheduled_accumulation",label:"Scheduled accumulation",weight:4}];
