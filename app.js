...
async function loadDashboard() {
  ...
  (backtests || []).forEach((test) => { const current = backtestSummary.get(test.bot_id) || { seconds: 0, runs: 0, signalOnlyRuns: 0, signals: 0, pnl: 0, capital: 0, profitPct: null }; current.seconds += Number(test.duration_seconds); current.runs++; if (test.status === "signal_only") { current.signalOnlyRuns++; current.signals += Number(test.signal_count || 0); } if (test.net_pnl != null) { current.pnl += Number(test.net_pnl); current.capital += Number(test.initial_capital); current.profitPct = current.capital ? current.pnl / current.capital * 100 : null; } backtestSummary.set(test.bot_id, current); });
...
function renderBots() {
  ...
  const row = (bot, isOption) => { const summary = backtestSummary.get(bot.id) || { seconds: 0, runs: 0, signalOnlyRuns: 0, signals: 0, profitPct: null }; const isOn = bot.status === "active"; const performance = isOption ? (summary.signalOnlyRuns ? `${summary.signals} triggers` : "Not tested") : (summary.profitPct == null ? "Not tested" : pct(summary.profitPct)); const coverage = `${formatDuration(summary.seconds)} · ${summary.runs} run${summary.runs === 1 ? "" : "s"}`; return `<div class="bot-row"><div><div class="bot-name">${escapeHtml(bot.name)}</div><div class="subtle">${escapeHtml(bot.symbol)} · ${escapeHtml(bot.bot_type.replaceAll("_", " "))}</div></div><button class="bot-toggle ${isOn ? "on" : ""}" data-toggle-bot="${bot.id}" role="switch" aria-checked="${isOn}"><span></span>${isOn ? "ON" : "OFF"}</button><div><div class="subtle">${isOption ? "SIGNAL COVERAGE" : "BACKTEST PROFIT"}</div><strong class="${!isOption && summary.profitPct > 0 ? "profit" : !isOption && summary.profitPct < 0 ? "loss" : isOption && summary.signalOnlyRuns ? "signal-only" : ""}">${performance}</strong><div class="subtle">${coverage}</div></div><div><div class="subtle">MAX ${isOption ? "RISK" : "ALLOCATION"}</div>${money(bot.max_allocation)}</div><div class="row-actions"><button class="secondary" data-bot-details="${bot.id}">Details</button><button class="primary" data-backtest="${bot.id}">${isOption ? "Test signals" : "Backtest"}</button><button class="delete-button" data-delete-bot="${bot.id}" aria-label="Delete ${escapeHtml(bot.name)}">×</button></div></div>`; };
  const stocks = bots.filter((bot) => bot.asset_class !== "option"); const options = bots.filter((bot) => bot.asset_class === "option").sort((a, b) => (backtestSummary.get(b.id)?.signals || 0) - (backtestSummary.get(a.id)?.signals || 0));
  return `<div class="bot-groups">${stocks.length ? `<section><div class="group-heading"><div><h3>Stock bots</h3><p>Ranked by weighted historical return</p></div><span>${stocks.length}</span></div><div class="bot-list">${stocks.map((bot) => row(bot, false)).join("")}</div></section>` : ""}${options.length ? `<section><div class="group-heading"><div><h3>Option strategies</h3><p>Ranked by historical underlying-signal count</p></div><span>${options.length}</span></div><div class="option-explainer">Option history measures entry-signal frequency and market context, not hypothetical spread P&amp;L.</div><div class="bot-list">${options.map((bot) => row(bot, true)).join("")}</div></section>` : ""}</div>`;
}
...
async function showBotDetails(botId) {
  ...
  const [{ data: steps }, { data: spread }, { data: recentTests }] = await Promise.all([
    ...
    supabase.from("bg_backtests").select("id,status,start_at,end_at,duration_seconds,net_pnl,return_pct,signal_count,market_regime,market_return_pct,volatility_label,created_at").eq("bot_id", bot.id).in("status", ["completed", "signal_only"]).order("created_at", { ascending: false }).limit(8),
  ]);
  ...
  const history = recentTests?.length ? `<div class="test-history">${recentTests.map((test) => `<div><div><strong>${new Date(test.start_at).toLocaleDateString()} – ${new Date(test.end_at).toLocaleDateString()}</strong><span>${escapeHtml(test.market_regime || "Unclassified")} · ${escapeHtml(test.volatility_label || "Unknown")} volatility · market ${pct(test.market_return_pct)}</span></div><div class="test-outcome"><strong>${test.status === "signal_only" ? `${test.signal_count} signals` : money(test.net_pnl)}</strong><span>${test.return_pct == null ? "Signal coverage" : `${pct(test.return_pct)} bot return`}</span></div></div>`).join("")}</div>` : `<div class="empty compact"><strong>No completed backtests yet</strong></div>`;
...
function switchView(view) {
  ...
  content.innerHTML = `<div class="section-head"><h3>Broker connections</h3></div><div class="card connection-card"><p>Connect an Alpaca paper account using credentials created in your Alpaca dashboard.</p><button class="primary" data-connect>Connect Alpaca</button></div><div class="section-head"><h3>Paper execution capacity</h3></div><div class="settings-grid"><div class="card"><span class="eyebrow">PLANNED SCHEDULE</span><h3>Every five minutes</h3><p>One shared worker will process all ON bots, group them by user and symbol, and reuse market-data requests.</p></div><div class="card"><span class="eyebrow">ESTIMATED USAGE</span><h3>≈1,638 calls/month</h3><p>About 0.33% of the current 500,000 monthly Free Plan function allowance during regular market hours.</p></div><div class="card"><span class="eyebrow">DATABASE POLICY</span><h3>Meaningful events only</h3><p>Store signals, orders, fills, errors, and state changes. Avoid a row for every failed condition check and retain detailed events for 30 days.</p></div><div class="card"><span class="eyebrow">INITIAL GUARDRAILS</span><h3>25 ON bots per user</h3><p>Process no more than 100 bots per invocation, fetch option chains only after an underlying signal, and pause repeated Alpaca errors.</p></div></div><div class="section-head"><h3>Backtest data notes</h3></div><div class="card note-card"><h3>Why option bots show “Signal-only”</h3><p>Stock bots can be simulated directly from historical bars. Credit-spread bots select contracts by delta at entry time, but Alpaca’s historical option data does not provide a historical chain of Greeks that can reproduce that selection exactly. BotGarden records when the underlying strategy would have triggered without inventing spread prices or P&L.</p></div>`;
}
...
async function createBulkOptionBot(index) {
  const bias = index % 2 ? "bearish" : "bullish"; const selected = randomizedOptionStrategy(pick(OPTION_STRATEGIES.filter((strategy) => strategy.family === "credit_spread" && strategy.bias === bias)), "balanced");
