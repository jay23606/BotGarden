...
function renderBots() {
  ...
  return `<div class="bot-list">${bots.map((bot) => { const summary = backtestSummary.get(bot.id) || { seconds: 0, runs: 0, signalOnlyRuns: 0, signals: 0, profitPct: null }; const isOn = bot.status === "active"; const performance = summary.profitPct != null ? pct(summary.profitPct) : summary.signalOnlyRuns ? "Signal-only" : "Not tested"; const coverage = summary.signalOnlyRuns ? `${summary.signals} signals · ${formatDuration(summary.seconds)}` : `${formatDuration(summary.seconds)} · ${summary.runs} run${summary.runs === 1 ? "" : "s"}`; return `<div class="bot-row"><div><div class="bot-name">${escapeHtml(bot.name)}</div><div class="subtle">${escapeHtml(bot.symbol)} · ${escapeHtml(bot.bot_type.replaceAll("_", " "))}</div></div><button class="bot-toggle ${isOn ? "on" : ""}" data-toggle-bot="${bot.id}" role="switch" aria-checked="${isOn}"><span></span>${isOn ? "ON" : "OFF"}</button><div><div class="subtle">BACKTEST PROFIT</div><strong class="${summary.profitPct > 0 ? "profit" : summary.profitPct < 0 ? "loss" : summary.signalOnlyRuns ? "signal-only" : ""}">${performance}</strong><div class="subtle">${coverage}</div></div><div><div class="subtle">MAX ALLOCATION</div>${money(bot.max_allocation)}</div><div class="row-actions"><button class="secondary" data-bot-details="${bot.id}">Details</button><button class="primary" data-backtest="${bot.id}">Backtest</button><button class="delete-button" data-delete-bot="${bot.id}" aria-label="Delete ${escapeHtml(bot.name)}">×</button></div></div>`; }).join("")}</div>`;
}
