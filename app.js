...
function renderRecentRealized(results = []) {
  const recent = results.slice(0, 10), winners = results.filter((trade) => Number(trade.realized_pnl) > 0), gains = winners.reduce((sum, trade) => sum + Number(trade.realized_pnl || 0), 0), net = results.reduce((sum, trade) => sum + Number(trade.realized_pnl || 0), 0);
  ...
  const rows = recent.map((trade) => { const pnl = Number(trade.realized_pnl || 0), symbols = (trade.symbols || []).join(" · ") || "Position", optionLike = (trade.symbols || []).some((symbol) => /\d{6}[CP]\d{8}$/.test(symbol)); return `<div class="realized-row"><div><strong>${escapeHtml(trade.bot_name)}</strong><span>${escapeHtml(optionLike ? `${symbols} · option legs closed` : symbols)}</span></div><div><span>Closed</span><strong>${new Date(trade.closed_at).toLocaleString()}</strong></div><div><span>${optionLike ? "Matched quantity" : "Entry → exit"}</span><strong>${optionLike ? Number(trade.quantity).toLocaleString(undefined,{maximumFractionDigits:4}) : `${money(trade.average_entry)} → ${money(trade.average_exit)}`}</strong></div><strong class="${pnl > 0 ? "profit" : pnl < 0 ? "loss" : ""}">${pnl >= 0 ? "+" : ""}${money(pnl)}</strong><button class="icon-button" data-bot-details="${trade.bot_id}" title="View bot details" aria-label="View ${escapeHtml(trade.bot_name)} details">›</button></div>`; }).join("");
  return `<section class="realized-activity"><div class="section-head"><div><h3>What changed paper equity?</h3><span class="subtle">Most recent realized bot-attributed closing fills</span></div><div class="realized-totals"><span>Profitable closes <b>${winners.length}</b></span><span>Gross gains <b class="profit">+${money(gains)}</b></span><span>Net shown <b class="${net>0?"profit":net<0?"loss":""}">${net>=0?"+":""}${money(net)}</b></span></div></div><div class="card realized-list">${rows}</div><p class="subtle realized-note">Realized P&amp;L is reconstructed from FIFO bot-owned Alpaca fills. Changes from open-position prices remain in Unrealized P&amp;L above.</p></section>`;
}
