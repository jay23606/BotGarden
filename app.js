...
import { assessPruneEvidence } from "./evaluation.js";
...
let botStatuses = new Map();
let currentView = "dashboard";
let activityFilter = "equity";
let activityLoadId = 0;
...
function renderRecentRealized(results = []) {
  const recent = results.slice(0, 10), winners = recent.filter((trade) => Number(trade.realized_pnl) > 0), gains = winners.reduce((sum, trade) => sum + Number(trade.realized_pnl || 0), 0), net = recent.reduce((sum, trade) => sum + Number(trade.realized_pnl || 0), 0);
  if (!recent.length) return `<section class="realized-activity"><div class="section-head"><div><h3>Recent realized bot results</h3><span class="subtle">Closed positions attributed from Alpaca fills</span></div></div><div class="card empty compact"><strong>No attributed closing trades yet</strong><span>Profits and losses will appear here after a BotGarden position is closed.</span></div></section>`;
  const rows = recent.map((trade) => { const pnl = Number(trade.realized_pnl || 0), tradeReturn=Number(trade.return_pct), symbols = (trade.symbols || []).join(" · ") || "Position", optionLike = (trade.symbols || []).some((symbol) => /\d{6}[CP]\d{8}$/.test(symbol)),reason=String(trade.exit_reason||"closing_fill").replaceAll("_"," "),overlap=Number(trade.concurrent_bot_count||0),quantity=Number(trade.quantity).toLocaleString(undefined,{maximumFractionDigits:8}),pnlText=Math.abs(pnl)>0&&Math.abs(pnl)<.005?(pnl<0?"< $0.01 loss":"< $0.01 gain"):`${pnl>=0?"+":""}${money(pnl)}`; return `<div class="realized-row" title="${escapeHtml(trade.exit_message||`Exit reason: ${reason}`)}"><div><strong>${escapeHtml(trade.bot_name)}</strong><span>${escapeHtml(optionLike ? `${symbols} · option legs closed` : symbols)}${overlap>1?` · ${overlap} bots shared this symbol`:""}</span></div><div><span>Closed · ${escapeHtml(reason)}</span><strong>${new Date(trade.closed_at).toLocaleString()}</strong></div><div><span>${optionLike ? "Matched quantity" : "Entry → exit"}</span><strong>${optionLike ? quantity : `${money(trade.average_entry)} → ${money(trade.average_exit)}`}</strong><small>${optionLike?"":`${quantity} units · `}${Number.isFinite(tradeReturn)?`${tradeReturn>=0?"+":""}${pct(tradeReturn)}`:"Return unavailable"}</small></div><strong class="${pnl > 0 ? "profit" : pnl < 0 ? "loss" : ""}">${pnlText}</strong><button class="icon-button" data-bot-details="${trade.bot_id}" title="View bot details" aria-label="View ${escapeHtml(trade.bot_name)} details">›</button></div>`; }).join("");
  return `<section class="realized-activity"><div class="section-head"><div><h3>What changed paper equity?</h3><span class="subtle">The ${recent.length} visible bot-attributed closing fills</span></div><div class="realized-totals"><span>Rows shown <b>${recent.length}</b></span><span>Profitable closes <b>${winners.length}</b></span><span>Visible gross gains <b class="profit">+${money(gains)}</b></span><span>Visible net <b class="${net>0?"profit":net<0?"loss":""}">${net>=0?"+":""}${money(net)}</b></span></div></div><div class="card realized-list">${rows}</div><p class="subtle realized-note">Every total above uses exactly the visible rows. Realized P&amp;L is reconstructed from FIFO bot-owned Alpaca fills; same-symbol overlap means several bots independently held the same market move. Open-position changes remain in Unrealized P&amp;L.</p></section>`;
}

function workerHealthBlock(operational = {}, portfolio = null) {
  ...
  const latestRuns = Object.keys(expected).map((mode) => (operational.worker_runs || []).find((run) => run.worker_mode === mode)).filter(Boolean), runRows = latestRuns.map((run) => `<div class="worker-run-row"><strong>${escapeHtml(labels[run.worker_mode] || run.worker_mode)}</strong><span>${Number(run.checked_count || 0)} checked · ${Number(run.submitted_count || 0)} submitted · ${Number(run.exit_count || 0)} exits</span><b class="${Number(run.error_count) ? "loss" : "profit"}">${Number(run.error_count || 0)} errors · ${run.duration_ms == null ? "running" : `${(Number(run.duration_ms) / 1000).toFixed(1)}s`}</b></div>`).join(""), issues = operational.reconciliation_issues || [], issueRows = issues.slice(0, 6).map((issue) => `<div class="health-issue ${escapeHtml(issue.severity)}"><div><strong>${escapeHtml(issue.symbol || "Order attribution")}</strong><span>${escapeHtml(issue.classification.replaceAll("_", " "))} · first seen ${new Date(issue.first_seen_at).toLocaleString()}</span></div><b>${issue.details?.unmanaged_quantity == null ? escapeHtml(String(issue.details?.count || "Review")) : `${Number(issue.details.unmanaged_quantity).toLocaleString(undefined,{maximumFractionDigits:8})} qty`}</b></div>`).join(""), brokerRow = portfolio ? `<div class="worker-health-row"><div><strong>Alpaca account sync</strong><span>${portfolio.as_of ? `Last successful response ${new Date(portfolio.as_of).toLocaleString()}` : escapeHtml(portfolio.error || "No successful response yet")}</span></div><b class="${portfolio.as_of ? "profit" : "loss"}">${portfolio.as_of ? "Connected" : "Unavailable"}</b></div>` : "";
  return `<div class="card worker-health"><div class="section-head compact-head"><div><h3>System health</h3><span class="subtle">Durable worker runs, broker connectivity, and ownership reconciliation</span></div><span class="status ${operational.entries_paused ? "paused" : ""}">${operational.entries_paused ? "ENTRIES PAUSED" : "ENTRIES ENABLED"}</span></div>${rows}${brokerRow}${runRows ? `<div class="worker-run-list">${runRows}</div>` : ""}<div class="health-issue-head"><strong>Open reconciliation issues</strong><span>${issues.length}</span></div>${issueRows || `<div class="health-all-clear"><strong>No open ownership issues</strong><span>Broker and bot-attributed quantities currently reconcile.</span></div>`}${issues.length > 6 ? `<small class="subtle">+${issues.length - 6} additional issue${issues.length - 6 === 1 ? "" : "s"}</small>` : ""}</div>`;
}

function renderActionCenter(operational = {}) {
  const actions = [], add = (group, priority, tone, title, detail, action = "") => actions.push({ group, priority, tone, title, detail, action });
  if (operational.entries_paused) add("Needs attention",100,"warning","New entries are globally paused","Risk exits continue, but no bot can submit a new entry until you resume them.",`<button class="secondary" data-view="settings">Review controls</button>`);
  const expected = { "entry-runner": 12, "risk-monitor": 3 }; Object.entries(expected).forEach(([mode, minutes]) => { const beat = (operational.health || []).find((item) => item.worker_mode === mode), age = beat?.completed_at ? (Date.now() - new Date(beat.completed_at).valueOf()) / 60000 : Infinity; if (age > minutes) add("Needs attention",95,"danger",`${mode === "entry-runner" ? "Entry runner" : "Risk monitor"} needs attention`,beat ? `Last completed ${Math.floor(age)} minutes ago.` : "No completed heartbeat has been recorded yet.",`<button class="secondary" data-view="settings">View health</button>`); });
  const unattributed = Number(paperPerformanceMeta?.unattributed_fill_count || 0); if (unattributed) add("Needs attention",90,"danger",`${unattributed} unattributed Alpaca fill${unattributed === 1 ? "" : "s"}`,"Resolve attribution before relying on bot-level paper P&L.",`<button class="secondary" data-view="activity">Review Activity</button>`);
  [...botStatuses.entries()].filter(([,status])=>status.reason_code==="error").slice(0,3).forEach(([id,status])=>{const bot=bots.find(item=>item.id===id);if(bot)add("Needs attention",88,"danger",`${bot.name} reported an error`,status.message,`<button class="secondary" data-bot-details="${bot.id}">Inspect bot</button>`);});
  bots.filter((bot) => botMaturity(bot).id === "degraded").slice(0, 3).forEach((bot) => add("Consider pruning",85,"danger",`Review degraded bot: ${bot.name}`,botMaturity(bot).note,`<button class="secondary" data-bot-details="${bot.id}">Inspect evidence</button>`));
  bots.filter((bot)=>smartPruneAssessment(bot).eligible&&botMaturity(bot).id!=="degraded").slice(0,3).forEach((bot)=>add("Consider pruning",75,"warning",`${bot.name} meets prune rules`,smartPruneAssessment(bot).reasons.join(" · "),`<button class="secondary" data-bot-details="${bot.id}">Review first</button>`));
  bots.filter((bot) => ["candidate", "proven"].includes(botMaturity(bot).id) && bot.status !== "active").slice(0, 3).forEach((bot) => add("Ready to promote",70,"positive",`${bot.name} is ${botMaturity(bot).label.toLowerCase()} but OFF`,"Review its evidence and decide whether to resume controlled paper observation.",`<button class="secondary" data-bot-details="${bot.id}">Review evidence</button>`));
  bots.filter((bot) => botMaturity(bot).id === "experimental" && botConfidence(bot).coverageDays < 10).sort((a,b)=>botConfidence(b).score-botConfidence(a).score).slice(0, 3).forEach((bot) => { const confidence = botConfidence(bot), needed = Math.max(1, 10 - confidence.coverageDays); add("Gather evidence",40,"",`${bot.name} needs broader history`,`Test at least ${needed} additional unique market day${needed === 1 ? "" : "s"} before historical validation.`,`<button class="secondary" data-backtest="${bot.id}">Add coverage</button>`); });
  actions.sort((a,b)=>b.priority-a.priority); const groups=["Needs attention","Ready to promote","Consider pruning","Gather evidence"], visible=groups.map(group=>({group,items:actions.filter(item=>item.group===group).slice(0,4)})).filter(section=>section.items.length);
  return `<section class="today-center"><div class="section-head"><div><span class="eyebrow">BOT GARDENER</span><h3>Today’s decision workflow</h3></div><span class="subtle">Advisory only · evidence and operations, not profit promises</span></div>${visible.length ? `<div class="gardener-grid">${visible.map(section=>`<section class="gardener-column"><div class="gardener-heading"><strong>${escapeHtml(section.group)}</strong><span>${section.items.length}</span></div>${section.items.map((item)=>`<article class="action-item ${item.tone}"><div><strong>${escapeHtml(item.title)}</strong><p>${escapeHtml(item.detail)}</p></div>${item.action}</article>`).join("")}</section>`).join("")}</div>` : `<div class="card all-clear"><strong>No urgent actions</strong><span>Workers, attribution, evidence stages, and paper controls currently look orderly.</span></div>`}</section>`;
}
...
async function loadDashboard() {
  const [{ data: botData }, connection, { data: backtests }, portfolio, observedPerformance, operational, { data: statuses }] = await Promise.all([
    ...
    supabase.from("bg_bot_status").select("bot_id,reason_code,message,details,checked_at"),
  ]);
  paperPerformance = new Map((observedPerformance?.bots || []).map((item) => [item.bot_id, item]));
  paperPerformanceMeta = observedPerformance;
  botStatuses = new Map((statuses || []).map((status) => [status.bot_id, status]));
  backtestSummary = new Map();
  latestBacktest = new Map();
  (backtests || []).forEach((test) => { const current = backtestSummary.get(test.bot_id) || { seconds: 0, runs: 0, signalOnlyRuns: 0, signals: 0, trades: 0, wins: 0, losses: 0, positiveRuns: 0, evaluatedRuns: 0, maxDrawdown: 0, validationReturns: [], marketReturns: [], testedDates: new Set(), testedRegimes: new Set(), pnl: 0, capital: 0, profitPct: null, estimatedPnl: 0, estimatedCapital: 0, estimatedPct: null }; current.seconds += Number(test.duration_seconds); current.runs++; (test.daily_regimes || []).forEach((day) => { current.testedDates.add(day.date); if(day.regime)current.testedRegimes.add(day.regime); }); current.trades += Number(test.trade_count || 0); current.wins += Number(test.win_count || 0); current.losses += Number(test.loss_count || 0); current.maxDrawdown = Math.max(current.maxDrawdown, Number(test.max_drawdown_pct || 0)); const validation = test.status === "signal_only" ? test.walk_forward?.validation_estimated_return_pct : test.walk_forward?.validation_return_pct; if (validation != null) current.validationReturns.push(Number(validation)); if (test.market_return_pct != null) current.marketReturns.push(Number(test.market_return_pct)); const evaluated = test.status === "signal_only" ? test.estimated_return_pct : test.return_pct; if (evaluated != null) { current.evaluatedRuns++; if (Number(evaluated) > 0) current.positiveRuns++; } if (test.status === "signal_only") { current.signalOnlyRuns++; current.signals += Number(test.signal_count || 0); if (test.estimated_pnl != null) { current.estimatedPnl += Number(test.estimated_pnl); current.estimatedCapital += Number(test.initial_capital); current.estimatedPct = current.estimatedPnl / current.estimatedCapital * 100; } } if (test.net_pnl != null) { current.pnl += Number(test.net_pnl); current.capital += Number(test.initial_capital); current.profitPct = current.capital ? current.pnl / current.capital * 100 : null; } backtestSummary.set(test.bot_id, current); const latest = latestBacktest.get(test.bot_id); if (!latest || new Date(test.created_at) > new Date(latest.created_at)) latestBacktest.set(test.bot_id, test); });
  ...
  content.innerHTML = `
    ...
    ${renderRecentRealized(observedPerformance?.recent_realized || [])}
    <div class="section-head"><h3>30-day paper equity</h3></div>
    ${renderEquityHistory(portfolio?.history || [])}
    <div class="section-head"><h3>Portfolio risk health</h3><span class="subtle">Entry limits enforced · 75% gross, 20% per symbol, 3% daily loss</span></div>
    ...
    ${workerHealthBlock(operational, portfolio)}
...
function botDecisionSummary(bot) {
  if (bot.status !== "active") return `<div class="bot-decision muted"><b>Paused</b><span>This bot will not open new positions until switched ON.</span></div>`;
  const status = botStatuses.get(bot.id); if (!status) return `<div class="bot-decision"><b>Waiting</b><span>No scheduled evaluation has been recorded yet.</span></div>`;
  const details = status.details || {}, conditions = details.conditions || [], failed = conditions.find((condition) => !condition.passed), reason = String(status.reason_code || "waiting"), age = status.checked_at ? Math.max(0, Math.floor((Date.now() - new Date(status.checked_at).valueOf()) / 60000)) : null;
  let label = "Waiting", explanation = status.message || "Waiting for the next scheduled evaluation.";
  if (reason === "conditions_not_met" && failed) explanation = `${conditionDefinition(failed.type)[1]} is ${failed.measured || "not satisfied"}; it needs ${failed.required || "the configured threshold"}.`;
  else if (["risk_monitoring", "position_monitored"].includes(reason)) { label = "Holding"; const current = details.unrealized_pnl_pct; explanation = current == null ? "An attributed position is open; no exit threshold has been reached." : `Bot-owned return is ${current >= 0 ? "+" : ""}${pct(current)}; target ${bot.take_profit_pct == null ? "uses the strategy rule" : pct(bot.take_profit_pct)} and stop ${bot.stop_loss_pct == null ? "uses the strategy rule" : `-${pct(bot.stop_loss_pct)}`}.`; }
  else if (reason.includes("submitted")) { label = "Acted"; explanation = status.message; }
  else if (reason === "existing_exposure") explanation = "This bot already owns a position or pending order, so another entry is blocked.";
  else if (reason === "portfolio_risk_gate") { label = "Risk blocked"; explanation = status.message; }
  else if (reason === "market_closed") explanation = "The stock or option market is closed; evaluation resumes at the next open.";
  else if (reason === "error") { label = "Needs attention"; explanation = status.message; }
  return `<div class="bot-decision ${reason === "error" ? "error" : reason.includes("submitted") ? "acted" : ""}"><b>${escapeHtml(label)}</b><span>${escapeHtml(explanation)}</span>${age == null ? "" : `<small>${age ? `${age}m ago` : "just now"}</small>`}</div>`;
}

function botConfidence(bot, universe = bots) {
  const summary = backtestSummary.get(bot.id) || {}, paper = paperPerformance.get(bot.id) || {}, coverageDays = Number(summary.testedDates?.size || 0), observations = bot.asset_class === "option" ? Math.min(Number(summary.signals || 0), coverageDays * 3) : Math.min(Number(summary.trades || 0), coverageDays * 5), validationValues = summary.validationReturns || [], validationAverage = validationValues.length ? validationValues.reduce((sum, value) => sum + value, 0) / validationValues.length : null, validationPositiveRate=validationValues.length?validationValues.filter(value=>value>0).length/validationValues.length:0,regimeCount=Number(summary.testedRegimes?.size||0),clamp = (value, min, max) => Math.max(min, Math.min(max, value));
  ...
  const family = bot.start_condition?.generated_strategy, siblingCount = family ? universe.filter((item) => item.start_condition?.generated_strategy === family).length : 1, selectionPenalty = coverageDays < 20 && siblingCount > 5 ? clamp(Math.log2(siblingCount) * 1.5, 0, 8) : 0; let score = Math.round(clamp(evidence + validation + consistency + risk + paperScore - selectionPenalty, 0, 100)); if (fillCount < 10) score = Math.min(score, 74); if(validationValues.length<2||regimeCount<2)score=Math.min(score,59);if (bot.asset_class === "option" && fillCount < 10) score = Math.min(score, 55);
  const label = score >= 75 ? "High confidence" : score >= 60 ? "Promising" : score >= 40 ? "Experimental" : score >= 20 ? "Low evidence" : "Unproven";
  return { score, label, validationAverage, validationRuns:validationValues.length, validationPositiveRate, regimeCount, coverageDays, observations, fillCount, siblingCount, breakdown: { Evidence: evidence, "Unseen validation": validation, Consistency: consistency, "Drawdown control": risk, "Observed paper": paperScore, "Selection penalty": -selectionPenalty } };
}

function botMaturity(bot) {
  const confidence = botConfidence(bot), paper = paperPerformance.get(bot.id) || {}, paperPnl = Number(paper.total_pnl || 0), validated = confidence.coverageDays >= 20 && confidence.observations >= 20 && confidence.validationRuns >= 2 && confidence.validationAverage > 0 && confidence.validationPositiveRate >= .5 && confidence.regimeCount >= 2 && confidence.score >= 40;
...
function evidenceExplanation(bot) {
  const confidence = botConfidence(bot), maturity = botMaturity(bot), summary = backtestSummary.get(bot.id) || {}, paper = paperPerformance.get(bot.id) || {}, result = bot.asset_class === "option" ? summary.estimatedPct : summary.profitPct, components = Object.entries(confidence.breakdown).sort((a,b)=>Number(b[1])-Number(a[1])), strongest = components[0], weakest = components.at(-1), next = maturity.id === "degraded" ? "Keep OFF or reduce exposure while you inspect paper losses and unseen validation." : confidence.coverageDays < 20 ? `Backtest at least ${20-confidence.coverageDays} more unique market day${20-confidence.coverageDays===1?"":"s"}.` : confidence.validationRuns < 2 ? "Run another independent date range before promotion." : confidence.regimeCount < 2 ? "Test the bot in at least one additional market regime." : confidence.fillCount < 10 ? `Collect ${10-confidence.fillCount} more attributed paper fill${10-confidence.fillCount===1?"":"s"} before promotion.` : confidence.validationAverage == null || confidence.validationAverage <= 0 ? "Seek positive chronological holdout performance before promotion." : maturity.id === "proven" ? "Continue controlled paper observation; evidence supports the current stage but does not guarantee profit." : "Continue controlled paper observation and monitor drawdown consistency.";
  return `<h3 class="detail-title">Why this rank and stage?</h3><div class="evidence-explanation"><div class="evidence-verdict"><span>Current interpretation</span><strong>${escapeHtml(maturity.label)} · ${confidence.score}/100</strong><p>${escapeHtml(maturity.note)}</p></div><div><span>Strongest evidence</span><strong>${escapeHtml(strongest?.[0] || "None yet")}</strong><p>${strongest ? `${Number(strongest[1])>=0?"+":""}${Number(strongest[1]).toFixed(1)} score contribution` : "No evidence recorded"}</p></div><div><span>Largest limitation</span><strong>${escapeHtml(weakest?.[0] || "Evidence depth")}</strong><p>${weakest ? `${Number(weakest[1])>=0?"+":""}${Number(weakest[1]).toFixed(1)} score contribution` : "More independent observations are needed"}</p></div><div><span>Historical evidence</span><strong>${result == null ? "Not tested" : `${Number(result)>=0?"+":""}${pct(result)}`}</strong><p>${confidence.coverageDays} unique days · ${confidence.observations} observations</p></div><div><span>Observed paper evidence</span><strong class="${Number(paper.total_pnl)>0?"profit":Number(paper.total_pnl)<0?"loss":""}">${confidence.fillCount ? money(paper.total_pnl) : "No fills yet"}</strong><p>${confidence.fillCount} attributed fills · not a forecast</p></div><div class="evidence-next"><span>Best next decision</span><strong>${escapeHtml(next)}</strong></div></div>`;
}
...
function renderCrypto() {
  ...
  const pruneButton = content.querySelector("[data-prune-crypto]"); if (pruneButton) { pruneButton.textContent = `Smart prune & clean up (${pruneCount})`; pruneButton.title = "Removes crypto bots with sufficient evidence of underperformance; unproven bots are retained"; }
}
...
function smartPruneAssessment(bot) {
  const summary = backtestSummary.get(bot.id) || {}, confidence = botConfidence(bot), paper = paperPerformance.get(bot.id) || {}, result = bot.asset_class === "option" ? summary.estimatedPct : summary.profitPct;
  const marketReturns = summary.marketReturns || [], benchmarkReturn = marketReturns.length ? marketReturns.reduce((sum, value) => sum + value, 0) / marketReturns.length : null;
  const paperReturn = Number(bot.max_allocation) && Number(paper.fill_count || 0) ? Number(paper.total_pnl || 0) / Number(bot.max_allocation) * 100 : null;
  return assessPruneEvidence({
    assetClass: bot.asset_class, result, benchmarkReturn,
    coverageDays: confidence.coverageDays,
    evaluatedRuns: Number(summary.evaluatedRuns || 0),
    positiveRuns: Number(summary.positiveRuns || 0),
    validationAverage: confidence.validationAverage,
    validationRuns: confidence.validationRuns,
    maxDrawdown: Number(summary.maxDrawdown || 0),
    paperFillCount: Number(paper.fill_count || 0),
    paperPnl: Number(paper.total_pnl || 0),
    paperReturn,
  });
}
...
function updatePruneButton() {
  ...
  button.title = count ? `Remove ${count} evidence-backed underperformer${count === 1 ? "" : "s"}, cancel unmanaged orders, and close wholly unmanaged positions` : "No bots currently have sufficient evidence of underperformance; unproven bots are retained";
}
...
async function renderActivityPositions(orderAttribution = new Map(), loadId = activityLoadId, filter = activityFilter){
  const snapshot=await invoke("portfolio-snapshot",{}).catch(error=>({error:error.message,positions:[],pending_orders:[],fills:[]}));
  if(loadId!==activityLoadId||currentView!=="activity"||filter!==activityFilter)return;
  const slot=document.querySelector("#activity-broker-state");if(!slot)return;
  if(snapshot.error){slot.innerHTML=`<div class="callout reconciliation-alert"><strong>Broker activity could not be loaded</strong><br>${escapeHtml(snapshot.error)} <button class="secondary" id="retry-activity">Try again</button></div>`;$("#retry-activity")?.addEventListener("click",loadActivity);return;}
  const unique=(items,key)=>[...new Map(items.map(item=>[key(item),item])).values()];
  const positions=unique((snapshot.positions||[]).filter(position=>position.asset_class===filter),position=>`${position.asset_class}:${position.symbol}`),pending=unique((snapshot.pending_orders||[]).filter(order=>order.asset_class===filter),order=>order.id),fills=unique((snapshot.fills||[]).map(fill=>({fill,bot:orderAttribution.get(fill.order_id)})).filter(({fill,bot})=>fillAssetClass(fill,bot)===filter),({fill})=>fill.id||`${fill.order_id}:${fill.symbol}:${fill.side}:${fill.quantity}:${fill.price}:${fill.transaction_time}`).slice(0,20);
  const unattributedOrders=unique((snapshot.pending_orders||[]).filter(order=>!orderAttribution.has(order.id)),order=>order.id),unattributedFills=unique((snapshot.fills||[]).filter(fill=>!orderAttribution.has(fill.order_id)),fill=>fill.id||`${fill.order_id}:${fill.symbol}:${fill.transaction_time}`),warning=unattributedOrders.length||unattributedFills.length?`<div class="callout reconciliation-alert"><strong>External or unattributed broker activity</strong><br>${unattributedOrders.length} pending order${unattributedOrders.length===1?"":"s"} and ${unattributedFills.length} recent fill${unattributedFills.length===1?"":"s"} cannot currently be tied to a BotGarden bot.</div>`:"";
  const total=positions.reduce((sum,position)=>sum+Number(position.unrealized_pl||0),0),positionRows=positions.map(position=>`<div class="position-row"><div><strong>${escapeHtml(position.symbol)}</strong><span>${escapeHtml(position.side)} · ${Number(position.qty).toLocaleString(undefined,{maximumFractionDigits:8})}</span></div><div><span>Average entry</span><strong>${money(position.avg_entry_price)}</strong></div><div><span>Current price</span><strong>${money(position.current_price)}</strong></div><div><span>Market value</span><strong>${money(position.market_value)}</strong></div><div><span>Unrealized P&amp;L</span><strong class="${position.unrealized_pl>0?"profit":position.unrealized_pl<0?"loss":""}">${money(position.unrealized_pl)} · ${pct(position.unrealized_plpc)}</strong></div><button class="position-close" data-close-position="${escapeHtml(position.symbol)}">Close at market</button></div>`).join("");
  ...
  slot.innerHTML=`${warning}<section class="position-panel"><div class="section-head"><h3>Open ${filter==="equity"?"stock":filter} positions</h3><div><strong class="${total>0?"profit":total<0?"loss":""}">${money(total)}</strong> <span class="subtle">unrealized P&amp;L · ${snapshot.as_of?new Date(snapshot.as_of).toLocaleTimeString():"unavailable"}</span></div></div>${positionRows?`<div class="position-list">${positionRows}</div>`:`<div class="empty compact"><strong>No open ${filter} positions</strong></div>`}</section><section><div class="section-head"><h3>Pending orders</h3><span class="subtle">${pending.length} cancelable ${filter} order${pending.length===1?"":"s"}</span></div>${pendingRows?`<div class="pending-order-list">${pendingRows}</div>`:`<div class="empty compact"><strong>No pending ${filter} orders</strong></div>`}</section><section><div class="section-head"><h3>Recent broker fills</h3><span class="subtle">Latest 20 · attributed by Alpaca order ID</span></div>${fillRows?`<div class="fill-list">${fillRows}</div>`:`<div class="empty compact"><strong>No recent ${filter} fills</strong></div>`}</section>`;
}
async function loadActivity() {
  const loadId=++activityLoadId,filter=activityFilter;clearTimeout(activityTimer);content.innerHTML=`<div class="activity-summary"><div><span class="eyebrow">LATEST AUTOMATION CHECK</span><h3>Loading ${filter==="equity"?"stock":filter} activity…</h3><p>Refreshing bot decisions, open positions, pending orders, and recent broker fills.</p></div></div><div id="activity-broker-state"><div class="card empty compact"><strong>Loading broker activity…</strong><span>Keeping this state visible until Alpaca responds.</span></div></div>`;
  const [{ data: statuses }, { data: allActivityBots }, { data: activityOrders }, { data: activityTrades }, { data: activityRuns }, { data: orderEvents }] = await Promise.all([supabase.from("bg_bot_status").select("*").order("checked_at", { ascending: false }), supabase.from("bg_bots").select("id,name,symbol,status,asset_class").order("created_at", { ascending: false }),supabase.from("bg_orders").select("broker_order_id,bot_id,trade_id").not("broker_order_id","is",null).order("created_at",{ascending:false}).limit(500),supabase.from("bg_trades").select("id,run_id").limit(500),supabase.from("bg_bot_runs").select("id,bot_id").limit(500),supabase.from("bg_bot_events").select("bot_id,details").order("created_at",{ascending:false}).limit(500)]); const statusByBot = new Map((statuses || []).map((status) => [status.bot_id, status])),activityBots=(allActivityBots||[]).filter(bot=>bot.asset_class===activityFilter),botById=new Map((allActivityBots||[]).map(bot=>[bot.id,bot])),runBot=new Map((activityRuns||[]).map(run=>[run.id,run.bot_id])),tradeBot=new Map((activityTrades||[]).map(trade=>[trade.id,runBot.get(trade.run_id)])),orderAttribution=new Map((activityOrders||[]).map(order=>[order.broker_order_id,botById.get(order.bot_id||tradeBot.get(order.trade_id))]));
  if(loadId!==activityLoadId||currentView!=="activity"||filter!==activityFilter)return;
  ...
  content.innerHTML = `<div class="activity-summary"><div><span class="eyebrow">LATEST AUTOMATION CHECK</span><h3>Why each bot acted—or waited</h3><p>Entries are evaluated every five minutes. Open-position risk and exits are checked every minute. This page refreshes automatically every 30 seconds.</p></div><button class="secondary" id="refresh-activity">Refresh now</button></div><div id="activity-broker-state"><div class="card empty compact"><strong>Loading broker activity…</strong><span>Open positions and fills will remain in one synchronized view.</span></div></div><div class="decision-list">${cards || `<div class="empty"><h3>No bots configured</h3></div>`}</div>`; $("#refresh-activity")?.addEventListener("click", loadActivity); clearTimeout(activityTimer); activityTimer = setTimeout(() => document.querySelector('[data-view="activity"]')?.classList.contains("active") && loadActivity(), 30000);
  await renderActivityPositions(orderAttribution,loadId,filter);
}

function switchView(view) {
  currentView = view;
  if (view !== "activity") { clearTimeout(activityTimer); activityTimer = null; activityLoadId++; }
