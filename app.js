...
async function loadDashboard() {
  const [{ data: botData }, connection, { data: backtests }, portfolio, observedPerformance] = await Promise.all([
    ...
    getConnection(),
    supabase.from("bg_backtests").select("bot_id,status,duration_seconds,initial_capital,net_pnl,return_pct,signal_count,estimated_pnl,estimated_return_pct,daily_regimes,walk_forward,start_at,end_at,created_at").in("status", ["completed", "signal_only"]),
...
function renderBots() {
  ...
  if (securitiesFilter === "option") return options.length ? `<div class="bot-groups"><section><div class="group-heading"><div><h3>Option strategies</h3><p>Ranked by estimated historical replay return</p></div><span>${options.length}</span></div><div class="option-explainer">Estimated replay uses underlying moves, configured delta, premium, width, time decay, and a liquidity haircut. It is a low-confidence pruning aid—not actual historical option P&amp;L.</div><div class="bot-list">${options.map((bot) => row(bot, true)).join("")}</div></section></div>` : `<div class="empty"><h3>No option bots yet</h3><div>Create or randomize an option strategy to begin.</div></div>`;
  return stocks.length ? `<div class="bot-groups"><section><div class="group-heading"><div><h3>Stock bots</h3><p>Ranked by weighted historical return</p></div><span>${stocks.length}</span></div><div class="bot-list">${stocks.map((bot) => row(bot, false)).join("")}</div></section></div>` : `<div class="empty"><h3>No stock bots yet</h3><div>Create or randomize a stock strategy to begin.</div></div>`;
}
...
function switchView(view) {
  ...
  content.innerHTML = `<div class="section-head"><h3>Broker connections</h3></div><div class="card connection-card"><p>Connect an Alpaca paper account using credentials created in your Alpaca dashboard.</p><button class="primary" data-connect>Connect Alpaca</button></div><div class="section-head"><h3>Installable app</h3></div><div class="card connection-card"><div><h3>Install BotGarden</h3><p>Run in a standalone window with a home-screen icon and an offline application shell. Trading data still requires a connection.</p></div><button class="secondary" data-install-pwa>${matchMedia("(display-mode: standalone)").matches ? "Installed" : "Install app"}</button></div><div class="section-head"><h3>Paper execution capacity</h3></div><div class="settings-grid"><div class="card"><span class="eyebrow">TWO-SPEED SCHEDULE</span><h3>1-minute risk · 5-minute entries</h3><p>Open-position exits are checked every minute. New entries remain on completed strategy-bar boundaries to avoid reacting repeatedly to unfinished candles.</p></div><div class="card"><span class="eyebrow">OPTION EXECUTION</span><h3>Live chain + atomic spreads</h3><p>Contracts are selected from current Greeks and quotes. Vertical spreads enter and exit as atomic Alpaca multi-leg paper orders.</p></div><div class="card"><span class="eyebrow">ORDER SAFETY</span><h3>Risk and exposure gates</h3><p>Orders fail closed if liquidity, premium, width, risk, or contract checks fail. Existing positions and open orders block another entry.</p></div><div class="card"><span class="eyebrow">CONTROLLED EVOLUTION</span><h3>Challenge, test, then promote</h3><p>BotGarden keeps live configurations stable. Future evolution should create bounded child challengers and promote them only after matched replay, shadow, and minimum-evidence checks.</p></div></div><div class="section-head"><h3>Backtest data notes</h3></div><div class="card note-card"><h3>Why option bots show “Signal-only”</h3><p>Live option execution uses the current chain. Historical chain-and-Greeks snapshots are not available in the same form, so historical tests record underlying triggers without inventing option P&amp;L.</p></div>`;
}
