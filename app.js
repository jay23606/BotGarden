...
function switchView(view) {
  ...
  content.innerHTML = `<div class="section-head"><h3>Broker connections</h3></div><div class="card connection-card"><p>Connect an Alpaca paper account using credentials created in your Alpaca dashboard.</p><button class="primary" data-connect>Connect Alpaca</button></div><div class="section-head"><h3>Paper execution capacity</h3></div><div class="settings-grid"><div class="card"><span class="eyebrow">ACTIVE SCHEDULE</span><h3>Every five minutes</h3><p>The shared worker evaluates ON stock and option bots during Alpaca market hours. OFF bots cannot create new entries.</p></div><div class="card"><span class="eyebrow">OPTION EXECUTION</span><h3>Live chain + atomic spreads</h3><p>Contracts are selected from current Greeks and quotes. Vertical spreads are sent as one Alpaca multi-leg paper order.</p></div><div class="card"><span class="eyebrow">ORDER SAFETY</span><h3>Risk and exposure gates</h3><p>Orders fail closed if liquidity, premium, width, risk, or contract checks fail. Existing positions and open orders block another entry.</p></div><div class="card"><span class="eyebrow">CURRENT LIMIT</span><h3>Entry automation first</h3><p>The worker opens qualifying paper positions. Automated option profit, loss, and expiration exits are the next execution milestone.</p></div></div><div class="section-head"><h3>Backtest data notes</h3></div><div class="card note-card"><h3>Why option bots show “Signal-only”</h3><p>Live option execution uses the current chain. Historical chain-and-Greeks snapshots are not available in the same form, so historical tests record underlying triggers without inventing option P&amp;L.</p></div>`;
}
