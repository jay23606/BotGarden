...
document.addEventListener("click", (event) => {
  ...
  if (event.target.closest("[data-new-crypto-batch]")) showCryptoBatchFormV2();
...
function renderCrypto() {
  ...
  const rows = cryptoBots.map((bot) => {
    ...
    const family=bot.start_condition?.generated_strategy||"atr_adaptive_grid",label={crypto_dca:"DCA + safety orders",atr_adaptive_grid:"ATR-adaptive grid",crypto_smart_trailing:"Smart trailing reversal",crypto_scheduled_accumulation:"Volatility-aware accumulation"}[family]||family.replaceAll("_"," ");
