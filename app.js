...
function renderBots() {
  ...
  const stocks = bots.filter((bot) => bot.asset_class === "equity"), options = bots.filter((bot) => bot.asset_class === "option").sort((a, b) => (backtestSummary.get(b.id)?.estimatedPct ?? -Infinity) - (backtestSummary.get(a.id)?.estimatedPct ?? -Infinity));
...
function renderCrypto() {
  const cryptoBots = bots.filter((bot) => bot.asset_class === "crypto");
  const rows = cryptoBots.map((bot) => {
    ...
    return `<div class="bot-row"><div><div class="bot-name">${escapeHtml(bot.name)}</div><div class="subtle">${escapeHtml(bot.symbol)} · ATR-adaptive grid · 24/7</div></div><button class="bot-toggle ${isOn ? "on" : ""}" data-toggle-bot="${bot.id}" role="switch" aria-checked="${isOn}"><span></span>${isOn ? "ON" : "OFF"}</button><div><div class="subtle">GRID REPLAY</div><strong class="${summary?.profitPct > 0 ? "profit" : summary?.profitPct < 0 ? "loss" : ""}">${result}</strong><div class="subtle">${formatDuration(summary?.seconds || 0)}</div></div>${stockSparkline(bot)}<div><div class="subtle">MAX ALLOCATION</div>${money(bot.max_allocation)}</div><div class="row-actions"><button class="icon-action" data-bot-details="${bot.id}" title="View grid details">${actionIcon("details")}</button><button class="icon-action test" data-backtest="${bot.id}" title="Replay this grid">${actionIcon("test")}</button><button class="delete-button" data-delete-bot="${bot.id}" title="Remove bot">×</button></div></div>`;
  }).join("");
  content.innerHTML = `<div class="crypto-hero"><div><span class="crypto-badge">ALPACA CRYPTO · 24/7</span><h2>Crypto has its own workspace</h2><p>Crypto bots run independently of stock-market hours. The first live strategy is an ATR-adaptive grid with bounded allocation and a fee-aware historical replay.</p><div class="crypto-note">Paper results are simulations. Crypto is volatile and availability depends on your jurisdiction and Alpaca account.</div></div><button class="crypto-action" data-new-crypto>↻ Random crypto grid</button></div><div class="section-head"><h3>Crypto bots</h3><button class="secondary" data-new-crypto>+ New random grid</button></div>${rows ? `<div class="bot-list">${rows}</div>` : `<div class="empty"><h3>No crypto bots yet</h3><div>Create an adaptive grid around a liquid USD crypto pair.</div><button class="primary" data-new-crypto>Create crypto grid</button></div>`}<div class="section-head"><h3>Strategy roadmap</h3></div><div class="strategy-roadmap"><div class="strategy-card live"><strong>Adaptive grid · Live</strong><span>Trades repeated range movement with ATR-sized boundaries.</span></div><div class="strategy-card"><strong>Portfolio rebalancer · Next</strong><span>Returns a crypto basket to target weights on drift thresholds.</span></div><div class="strategy-card"><strong>Smart trailing trade · Next</strong><span>Waits for a reversal after a pullback, then trails the exit.</span></div><div class="strategy-card"><strong>Pairs mean reversion</strong><span>Trades divergence between historically related assets.</span></div><div class="strategy-card"><strong>Scheduled accumulation</strong><span>Buys on a schedule with volatility-aware sizing.</span></div><div class="strategy-card"><strong>Webhook signals</strong><span>Accepts external strategy alerts with BotGarden risk controls.</span></div></div>`;
}

async function showCryptoGridForm() {
  modal.showModal();
  $("#modal-content").innerHTML = `<form id="crypto-grid-form"><div class="modal-head"><div><h3>Random ATR-adaptive crypto grid</h3><p>BotGarden chooses sensible parameters around recent volatility.</p></div><button type="button" class="icon-button" data-close-modal>×</button></div><div class="modal-body"><div class="callout">Major USD pairs only. Maximum allocation is a hard paper-trading budget; each grid order uses only a fraction of it.</div><div class="form-grid"><label>Maximum allocation<input name="risk" type="number" min="50" step="25" value="500" required></label><label>Replay days<input name="days" type="number" min="1" max="60" value="5" required></label></div><p class="form-message" id="crypto-message"></p></div><div class="modal-foot"><button type="button" class="secondary" data-close-modal>Cancel</button><button class="crypto-action" type="submit">Create and replay</button></div></form>`;
  $("#crypto-grid-form").addEventListener("submit", async (event) => {
    ...
    try {
      ...
      const asset = pick(candidates), risk = Number(data.get("risk")), levels = pick([6,7,8,9,10,12]), halfWidth = Math.max(.04, Math.min(.18, asset.atr_pct / 100 * pick([2.5,3,3.5]))), lower = asset.price * (1 - halfWidth), upper = asset.price * (1 + halfWidth), orderAmount = Math.max(10, Math.floor(risk / Math.ceil(levels / 2)));
      const { data: bot, error } = await supabase.from("bg_bots").insert({ user_id: session.user.id, name: `${asset.symbol.replace("/USD", "")} Adaptive Grid`, bot_type: "grid", status: "active", broker: "alpaca", environment: "paper", asset_class: "crypto", symbol: asset.symbol, direction: "long", max_allocation: risk, max_active_trades: levels, start_condition: { operator: "AND", conditions: [{ type: "immediate", timeframe: "15Min", parameters: {} }], generated_strategy: "atr_adaptive_grid", randomized_fields: { Pair: asset.symbol, "ATR %": asset.atr_pct.toFixed(2), "Grid levels": levels, "Range width": pct(halfWidth * 100), "Order amount": money(orderAmount) } }, take_profit_pct: null, stop_loss_pct: null, cooldown_seconds: 300, session_policy: "continuous" }).select().single(); if (error) throw error;
      const { error: gridError } = await supabase.from("bg_grid_configs").insert({ bot_id: bot.id, user_id: session.user.id, lower_price: lower, upper_price: upper, grid_levels: levels, order_amount: orderAmount, spacing_mode: "geometric", recenter_enabled: true, fee_bps: 25 }); if (gridError) { await supabase.from("bg_bots").delete().eq("id", bot.id); throw gridError; }
      button.textContent = "Replaying…"; try { await autoBacktest(bot.id, Number(data.get("days"))); } catch (error) { console.warn("Crypto replay failed", error); } modal.close(); await loadDashboard(); switchView("crypto");
