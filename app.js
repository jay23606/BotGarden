...
function evidenceExplanation(bot) {
  const confidence = botConfidence(bot), maturity = botMaturity(bot), summary = backtestSummary.get(bot.id) || {}, paper = paperPerformance.get(bot.id) || {}, result = bot.asset_class === "option" ? summary.estimatedPct : summary.profitPct, components = Object.entries(confidence.breakdown).sort((a,b)=>Number(b[1])-Number(a[1])), strongest = components[0], weakest = components.at(-1), next = maturity.id === "degraded" ? "Keep OFF or reduce exposure while you inspect paper losses and unseen validation." : confidence.coverageDays < 10 ? `Backtest at least ${10-confidence.coverageDays} more unique market day${10-confidence.coverageDays===1?"":"s"}.` : confidence.fillCount < 10 ? `Collect ${10-confidence.fillCount} more attributed paper fill${10-confidence.fillCount===1?"":"s"} before promotion.` : confidence.validationAverage == null || confidence.validationAverage <= 0 ? "Seek a positive chronological holdout before promotion." : maturity.id === "proven" ? "Continue controlled paper observation; evidence supports the current stage but does not guarantee profit." : "Continue controlled paper observation and monitor drawdown consistency.";
