...
async function loadDashboard() {
  const [{ data: botData }, connection, { data: backtests }, portfolio, observedPerformance, operational, { data: statuses }] = await Promise.all([
    ...
    getConnection(),
    supabase.from("bg_backtests").select("bot_id,status,duration_seconds,initial_capital,net_pnl,return_pct,max_drawdown_pct,trade_count,win_count,loss_count,signal_count,estimated_pnl,estimated_return_pct,daily_regimes,walk_forward,execution_model_version,start_at,end_at,created_at").in("status", ["completed", "signal_only"]).gte("execution_model_version",2),
