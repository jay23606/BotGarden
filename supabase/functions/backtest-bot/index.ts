...
Deno.serve(async (request) => {
  ...
  try {
    ...
    for (let i = 30; i < bars.length; i++) {
      ...
      let matches = conditions.length && (bot.start_condition.operator === "OR" ? conditions.some((c: any) => conditionMatches(c, bars, i, cache)) : conditions.every((c: any) => conditionMatches(c, bars, i, cache))); if(bot.start_condition?.generated_strategy==="scheduled_accumulation"){const now=new Date(bars[i].t).valueOf(),interval=Number(bot.cooldown_seconds||86400)*1000;matches=matches&&(!lastScheduledAt||now-lastScheduledAt>=interval);if(matches)lastScheduledAt=now;}
      ...
      const bar = bars[i];
      if (!position && matches) { const installments=bot.start_condition?.generated_strategy==="scheduled_accumulation"?Number(bot.start_condition?.strategy_config?.installments||1):1;const amount = Math.min(cash, Number(steps?.[0]?.order_amount || Number(bot.max_allocation)/installments)); position = { entryAt: bar.t, base: bar.c, qty: amount / bar.c, cost: amount, next: 1 }; cash -= amount; }
      else if(position && matches && bot.start_condition?.generated_strategy==="scheduled_accumulation" && cash>0){const installments=Number(bot.start_condition?.strategy_config?.installments||1),amount=Math.min(cash,Number(bot.max_allocation)/installments);position.qty+=amount/bar.c;position.cost+=amount;cash-=amount;}
      if (position) {
