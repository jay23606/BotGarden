...
Deno.serve(async (request) => {
  ...
  try {
    ...
    for (let i = 30; i < bars.length; i++) {
      ...
      const bar = bars[i];
      if (!position && matches) { const installments=scheduled?Number(bot.start_condition?.strategy_config?.installments||1):1,scale=scheduled?Number(bot.start_condition?.strategy_config?.volatility_scale||1)*Number(bot.start_condition?.strategy_config?.accelerated_dip_size||1):1;const amount = Math.min(cash, Number(steps?.[0]?.order_amount || Number(bot.max_allocation)/installments*scale)); position = { entryAt: bar.t, base: bar.c, qty: amount / bar.c, cost: amount, next: 1 }; cash -= amount; }
