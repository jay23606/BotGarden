...
Deno.serve(async (request) => {
  ...
  const summary = { checked: 0, matched: 0, submitted: 0, optionOrders: 0, optionValidated: 0, skipped: 0, errors: 0 };
  ...
  for (const bot of bots || []) {
    ...
    try {
      ...
      const clock = await alpaca("https://paper-api.alpaca.markets/v2/clock", apiKey, apiSecret); if (!clock.is_open && !validateOptions) { summary.skipped++; continue; }
      const [positions, openOrders] = await Promise.all([alpaca("https://paper-api.alpaca.markets/v2/positions", apiKey, apiSecret), alpaca("https://paper-api.alpaca.markets/v2/orders?status=open&nested=true&limit=500", apiKey, apiSecret)]); const hasExposure = positions.some((p: any) => p.symbol === bot.symbol || (bot.asset_class === "option" && p.symbol.startsWith(bot.symbol))) || openOrders.some((o: any) => o.symbol === bot.symbol || o.legs?.some((leg: any) => leg.symbol?.startsWith(bot.symbol))); if (hasExposure && !validateOptions) { summary.skipped++; continue; }
